    ...
    // إنشاء عنصر كرت افتراضي لعرضه عند الحفظ
    const tempCard = document.createElement('div');
    tempCard.className = 'card temp-card';
    tempCard.style.display = 'none'; // نخفيه عن العرض
    ...
    tempCard.innerHTML = `
                ...
                <div class="card-details">
                    ...
                    <div class="detail-row">
                        <span class="detail-label">مدة الصلاحية:</span>
                        <span class="detail-value">${sale.cardData?.uptime || 'غير محدد'}</span>
                    </div>
                </div>
    ...
    const qrContainer = tempCard.querySelector('.qr-code');
    new QRCode(qrContainer, {
    text: loginUrl,
    width: 120,
    height: 120,
    ...
    saleItem.innerHTML = `
                ...
                <div class="sale-actions">
                    <button class="btn-primary save-sale-btn" data-index="${index}">حفظ الكرت</button>
    ...
    saleItem.querySelector('.save-sale-btn').addEventListener('click', function() {
    saveCardAsImage(tempCard, sale.type, sale.username);
});
    ...
    // إزالة العنصر المؤقت إذا كان من نوع temp-card
    if (cardElement.classList.contains('temp-card')) {
    setTimeout(() => {
